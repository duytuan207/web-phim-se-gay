...
function hoanthanh($hoanthanh) {
	$html = "<select name=hoanthanh>".
		"<option value=Hoàn Thành".(($hoanthanh==Hoàn Thành)?' selected':'').">Hoàn Thành</option>".
		        "<option value=Chưa Hoàn Thành".(($hoanthanh==Chưa Hoàn Thành)?' selected':'').">Chưa Hoàn Thành</option>".
		        "<option value=Trailer".(($hoanthanh==Trailer)?' selected':'').">Trailer</option>".
		"</select>";
	return $html;
}
...
function acp_quick_add_film2($new_film,$name_real,$tapphim,$new_film_img,$actor,$year,$time,$area,$director,$cat,$info,$country,$file_type,$bo_le,$key,$des,$imgbn,$tag,$trang_thai,$imdb,$hoanthanh) {
	global $mysql, $tb_prefix;
	...
	if ($mysql->num_rows($q)) {
		...
		$mysql->query("UPDATE ".$tb_prefix."user SET user_point = user_point + 1 WHERE user_id = '".$_SESSION['admin_id']."'");
	    $mysql->query("INSERT INTO ".$tb_prefix."film (film_name,film_name_real,film_tapphim,film_name_ascii,film_img,film_actor,film_actor_ascii,film_year,film_time,film_area,film_director,film_director_ascii,film_cat,film_info,film_country,film_server,film_lb,film_key,film_des,film_imgbn,film_upload,film_tag,film_tag_ascii,film_trangthai,film_imdb,film_hoanthanh) VALUES ('".check_name($new_film)."','".check_name($name_real)."','".$tapphim."','".strtolower(get_ascii(check_name($new_film)))."','".$new_film_img."','".check_name($actor)."','".strtolower(get_ascii(check_name($actor)))."','".$year."','".$time."','".$area."','".check_name($director)."','".strtolower(get_ascii(check_name($director)))."','".$cat."','".queryspecail($info)."','".$country."','".$file_type."','".$bo_le."','".$keyw."','".$des."','".$imgbn."','".$_SESSION['admin_id']."','".$tag."','".strtolower(get_ascii($tag))."','".$trang_thai."','".$imdb."','".$hoanthanh."')");
		$film = $mysql->insert_id();
	}
	return $film;
}
