...
if(strpos($url , 'picasaweb') !== false){
	...
	if (count($match[0]) > 0)
	{
		foreach($match[0] as $item)
		{
			...
			if ($itemJS['height']>=700 && $itemJS['height'] < 900 && !isset($itemhd)) $itemhd = $itemJS['url'];		
			if ($itemJS['height']>=900 && !isset($itemfhd)) $itemfhd = $itemJS['url'];				
		}
		if (!isset($itemmedium))
		{
...
elseif(strpos($url , 'youtube') !== false){
	...
	if($fmturlmaps[38]) $itemfhd = $fmturlmaps[38];
}
elseif(strpos($url, 'tv.zing.vn')){
	...
	if($data['response']['download_url']['Video1080']) $itemfhd = reset(explode('?', "http://".$data['response']['download_url']['Video1080']));
}
